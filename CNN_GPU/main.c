...
#define NUMERO_DE_IMAGENS_PARA_TREINAR 58000
...
int main() {
	...
	CnnAddConvLayer(c, 1, 5, 5);
	//CnnAddPoolLayer(c, 2, 2);
	CnnAddConvLayer(c, 1, 1, 3);
	CnnAddConvLayer(c, 1, 2, 4);
	CnnAddConvLayer(c, 1, 1, 2);
		...
		for (int caso = 0; caso < limiteImages; caso++) {
			...
			if(stop== 'i') {
				printf("\t imagem %d / %d tempo estimado para fim da epoca %fs\n\testima-se %f m para terminar o treinamento \n", caso, limiteImages,
				       ...
				       ((((double) (clock() - initTimeALL)) / 1000.0) / (limiteImages*epoca+caso + 1.0)) * ((limiteImages - caso)+limiteImages*(maximoEpocas-epoca-1))/60.0);
				stop = 0;
			}
