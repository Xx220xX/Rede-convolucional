...
Camada createBatchNorm(WrapperCL *cl, cl_command_queue queue, Params params,
	...
	c->media = newTensor(cl->context,queue, 1, 1, inz, error);
	c->somaDiferenca = newTensor(cl->context,queue, 1, 1, inz, error);
	c->variancia = newTensor(cl->context,queue, 1, 1, inz, error);
	c->gradVariancia = newTensor(cl->context,queue, 1, 1, inz, error);

	c->Y = newTensor(cl->context,queue, 1, 1, inz, error);
	c->B = newTensor(cl->context,queue, 1, 1, inz, error);
	c->gradY = newTensor(cl->context,queue, 1, 1, inz, error);
	c->gradB = newTensor(cl->context,queue, 1, 1, inz, error);

	c->diferenca = newTensor(cl->context,queue, inx, iny, inz, error);
	c->diferencaquad = newTensor(cl->context,queue, inx, iny, inz, error);
	c->norma = newTensor(cl->context,queue, inx,iny, inz, error);
