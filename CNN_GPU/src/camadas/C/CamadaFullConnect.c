...
Camada createFullConnect(WrapperCL *cl, cl_command_queue queue, UINT inx, UINT iny, UINT inz, UINT tamanhoSaida,
	...
	c->z = newTensor(context, tamanhoSaida, 1, 1, error);
	c->dz = newTensor(context, tamanhoSaida, 1, 1, error);
	c->dz_old = newTensor(context, tamanhoSaida, 1, 1, error);
	c->pesos = newTensor(context, tamanhoSaida, inx * iny * inz, 1, error);
	...
	c->kernelfullcalcgrad1 = new_Kernel(cl->program,error, "fullcalcgrads1", 5, K_VOID_P, K_VOID_P, K_VOID_P, K_INT, K_INT);
	c->kernelfullcalcgrad2 = new_Kernel(cl->program,error, "fullcalcgrads2", 6, K_VOID_P, K_VOID_P, K_VOID_P, K_INT, K_INT,
	                                    K_INT);
	return (Camada) c;
}
...
void ativaFullConnect(CamadaFullConnect c) {
	kernel_run_recursive(&c->kernelfullfeed, c->super.queue, c->super.saida->x,
	                     *c->super.max_works,
...
void corrigePesosFullConnect(CamadaFullConnect c) {
	kernel_run_recursive(&c->kernelfullfixWeight, c->super.queue, c->super.saida->x, *c->super.max_works,
	                     &c->super.entrada->data,
	                     &c->pesos->data,
	                     &c->dz->data,
	                     &c->dz_old->data,
...
void calc_gradsFullConnect(CamadaFullConnect c, Tensor GradNext) {
	...
	kernel_run_recursive(&c->kernelfullcalcgrad1, c->super.queue, c->super.saida->x, *c->super.max_works,
	                     &c->dz->data, &GradNext->data, &c->z->data, &c->dfa);

	kernel_run_recursive(&c->kernelfullcalcgrad2, c->super.queue,
	                     c->super.entrada->x * c->super.entrada->y * c->super.entrada->z,
	                     *c->super.max_works,
	                     &c->dz->data,
