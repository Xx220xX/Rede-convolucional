...
Camada createConv(WrapperCL *cl, QUEUE queue, UINT passo, UINT lenFilter,
	...
	if (error->error)return (Camada) c;
	c->filtros = newTensor4D(cl->context,queue, lenFilter, lenFilter, inz, numeroFiltros, error);
	c->grad_filtros = newTensor4D(cl->context,queue, lenFilter, lenFilter, inz, numeroFiltros, error);
	c->grad_filtros_old = newTensor4D(cl->context,queue, lenFilter, lenFilter, inz, numeroFiltros, error);
