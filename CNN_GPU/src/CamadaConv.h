...
Camada createConv(WrapperCL *cl, UINT passo, UINT lenFilter, UINT numeroFiltros, UINT inx, UINT iny, UINT inz,
	...
	c->super.saida = newTensor(context, (inx - lenFilter) / passo + 1, (iny - lenFilter) / passo + 1, numeroFiltros,
	                           error);
	if (error->error)return (Camada) c;
...
void corrige_pesosConv(CamadaConv c) {
	...
	kernel_run_recursive(&c->kernelConvFixWeight, c->super.queue,
	                     c->tamanhoFiltro * c->tamanhoFiltro * c->super.entrada->z * c->numeroFiltros, max_works,
	                     &c->filtros->data, &c->grad_filtros->data,
	                     &c->grad_filtros_old->data,
	                     &c->super.parametros->hitLearn, &c->super.parametros->momento,
	                     &c->super.parametros->multiplicador, &c->super.parametros->decaimentoDePeso);
...
void calc_gradsConv(CamadaConv c, Tensor Gradnext) {
	...
	char zero = 0;
	clEnqueueFillBuffer(c->super.queue, c->grad_filtros->data, &zero, sizeof(char), 0,
	                    c->grad_filtros->bytes * c->numeroFiltros, 0, NULL, NULL);
	kernel_run_recursive(&c->kernelConvCalcGrads, c->super.queue,
	                     c->super.entrada->x * c->super.entrada->y * c->super.entrada->z, max_works, &c->filtros->data,
	                     &c->grad_filtros->data,
	                     &c->super.entrada->data,
	                     &c->super.gradsEntrada->data, &Gradnext->data, &c->tamanhoFiltro, &c->filtros->z,
	                     &c->passo, &c->super.entrada->x, &c->super.entrada->y, &c->super.saida->x,
	                     &c->super.saida->y, &c->numeroFiltros);
...
Camada carregarConv(WrapperCL *cl, FILE *src, Tensor entrada, Params *params, GPU_ERROR *error) {
	...
	CamadaConv c = (CamadaConv) createConv(cl, passo, tamanhoFiltro, numeroFiltros, inx, iny, inz, entrada, params,
	                                       error, 0);
	double *data = callocdouble(c->tamanhoFiltro * c->tamanhoFiltro * c->super.entrada->z);
	cl_command_queue queue = clCreateCommandQueueWithProperties(cl->context, cl->device, NULL, &error->error);
	for (int a = 0; a < c->numeroFiltros; a++) {
