 ...
 * 4 bytes -> funcao de ativacao
 * 8 bytes -> w.x
 * 8 bytes -> w.y
 * 8 bytes -> w.z
 * 4 bytes -> w.size_element
 * w.length *w.size_element bytes -> data w
 * w.x *w.size_element bytes -> data b
 *
 * @param self camada
...
Camada CamadaFullConnect_new(Gpu gpu, Queue queue, P3d size_in, size_t tamanhoSaida, Tensor entrada,
	...
	self->fullCalcDWandFix = Kernel_news(gpu->program, "fullCalcDWandFix",
										 "Vector dz, Vector ds,"
										 "Vector z, Vector b, Vector db, int dfa, REAL hitlearn,\n"
