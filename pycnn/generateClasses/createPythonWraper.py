...
__dll = os.path.join(__dir, '../bin/{dll_var_name}')
...
print(f'from {file_load_dll}  import *',file=file)
print(f'from {file_structs}  import *',file=file)

from generateFunctions import *
for h in h_functions:
    if h.startswith('camada'):continue
